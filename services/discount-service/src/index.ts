...
import { createDiscountRoutes } from './routes/discount.routes';
...
// API routes
app.use('/api/v1/coupons', createCouponRoutes(container.getCouponController()));
app.use('/api/v1/promotions', createPromotionRoutes(container.getPromotionController()));
app.use('/api/v1/discounts', createDiscountRoutes(container.getDiscountController()));
...
// Start server
const PORT = parseInt(config.PORT, 10) || 3008;
const server = app.listen(PORT, async () => {
  logger.info(`Discount Service started on port ${PORT}`, {
    env: config.NODE_ENV,
    port: PORT,
  });
