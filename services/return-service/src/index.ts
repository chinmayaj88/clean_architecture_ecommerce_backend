...
import { createReturnRoutes } from './routes/return.routes';
...
// API routes
app.use('/api/v1/returns', createReturnRoutes(container.getReturnRequestController()));
...
// Start server
const PORT = parseInt(config.PORT, 10) || 3010;
const server = app.listen(PORT, async () => {
  logger.info(`Return Service started on port ${PORT}`, {
    env: config.NODE_ENV,
    port: PORT,
  });
